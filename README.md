...
在 `token` 分支中，我们引入了全新的加密协议，显著提升了通信安全性。

*   **随机 Session Key**：每次会话启动时，Client 都会生成一个全新的随机密钥。
*   **双层加密**：
    *   **握手阶段**：使用预共享 Token 的 MD5 值加密随机生成的 Session Key。
    *   **会话阶段**：使用协商好的随机 Session Key 加密所有后续通信。
*   **前向保密**：即使攻击者拥有源代码，只要未捕获握手包，就无法解密后续流量。
